...
def load_walls(maze_array: Union[list, np.ndarray], scale: float, walls_width: float):
    maze_array = np.array(maze_array)
    maze_info = {"x_min": 0,
                 "x_max": maze_array.shape[1] * scale,
                 "y_min": 0,
                 "y_max": maze_array.shape[1] * scale,
                 "obstacles": [],
                 "agent_reset_position": (" ".join([str(pos) for pos in np.argwhere(maze_array == 2)[0] * scale])
                                          + " 0 0 0 " + str(-0.5 * math.pi))
                 }
    ...
    while np.isin(1, maze_array):
        ...
        if len(horizontal_wall) > len(vertical_wall):
            ...
            obstacle = {
                ...
                "size": str(width) + " " + str(walls_width)
            }
            ...
            obstacle = {
                ...
                "type": "rectangle",
                "position": str(position_x) + " " + str(position_y),
                "size": str(walls_width) + " " + str(height)
            }
            maze_info["obstacles"].append(obstacle)
            vertical_wall_id += 1
    return maze_info
...
def generate_xml(map_name: str, robot_name: str, scale: float = 1., walls_height: float = 0.5, nb_markers: int = 0) -> \
    ...
    if "maze_array" in locals():
        # Load walls
        maze_info = load_walls(maze_array, scale, walls_width)
    ...
    add_wall(name="horizontal_wall_2",
    ...
    cam_pose_node.text = "{0} {1} {2} 0 {3} 0".format(maze_center_x, maze_center_y, maze_height * 2, math.pi / 2)
...
if __name__ == "__main__":
    from environments.robotic_environment_v2.simulations_assets.maps.four_rooms import maze_array
