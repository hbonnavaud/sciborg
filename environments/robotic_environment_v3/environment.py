...
class SingleRobotEnvironmentV3(GoalConditionedEnvironment):
    ...
    def get_observation(self, compute_collision=True):
        ...
        if self.real_world:
            pose = self.last_position_message.pose
        else:
            pose = self.last_position_message.pose.pose
        obs = np.array([
            pose.position.x,
            pose.position.y,
            pose.position.z,
    ...
    def step(self, action: int):
        ...
        Args:
            action: int, the action to send to the robot
        ...
        if action is not None:  # Handy if you want to control the robot with a telehop script in parallel
            ...
            if action == ActionType.FORWARD.value:
                self.velocity[0] = self.linear_velocity
...
if __name__ == "__main__":
    ...
    env = SingleRobotEnvironmentV3(map_name=RobotMapsIndex.EMPTY.value, use_lidar=True,
                                   real_time=False)
